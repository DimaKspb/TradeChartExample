package com.text.traderchart.chart.model
...
abstract class BaseSeries<out T : BaseQuote> {
    ...
    fun calcMinMax() {
        ...
        maxY = visibleScreenData.maxBy { it.getValue() }?.getValue() ?: 0.0
        minY = visibleScreenData.minBy { it.getValue() }?.getValue() ?: 0.0

        minX = visibleScreenData[0].getTime()
        maxX = visibleScreenData.last().getTime()
    }
}
